  ...
  return (
    <nav className="bg-white text-black py-4 shadow-sm border-b border-gray-200">
      <div className="max-w-7xl mx-auto flex items-center justify-between px-6">
        ...
        {/* Desktop Navigation */}
        <div className="hidden md:flex space-x-8 font-display text-lg">
          <Link to="/" className="flex items-center gap-1 hover:text-[#005b82]">
            Home
          </Link>
          {data.navbar.items.map((item, index) => (
            <Link
              key={index}
              to={item.link}
              className="flex items-center gap-1 hover:text-[#005b82]"
            >
              {item.label}
              <svg
